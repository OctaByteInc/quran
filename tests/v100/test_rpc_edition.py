...
def test_create_edition():
    ...
    assert res.data.id == edition.id
    assert res.data.language == edition.language
    assert res.data.name == edition.name.title()
    assert res.data.translator == edition.translator.title()
    assert res.data.type == 'Arabic'
    assert res.data.format == edition.format.title()
    assert res.data.direction == edition.direction
...
def test_get_all_edition():
    ...
    count = 0
    for edition in edition_stream.data:
...
def test_find_edition_by_id():
    ...
    assert edition.data.id == 'edition-1'
    assert edition.data.name == 'Edition-Name-1'
