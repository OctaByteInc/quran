...
class AyahService(ayah_rpc.AyahSvcServicer):
    ...
    def CreateAyah(self, request, context):
        ...
        return entity_proto.Ayah(**res.to_dict())
    ...
    def _ayah_response(self, response):
        ayah_response = ayah_proto.AyahResponse(ayah=entity_proto.Ayah(**response.ayah.to_dict()))

        if response.translation:
            ayah_response.translation.MergeFrom(entity_proto.Translation(**response.translation.to_dict()))
        if response.surah:
            ayah_response.surah.MergeFrom(entity_proto.Surah(**response.surah.to_dict()))
        if response.edition:
            ayah_response.edition.MergeFrom(entity_proto.Edition(**response.edition.to_dict()))
        if response.arabic_audio:
            ayah_response.arabic_audio.MergeFrom(entity_proto.Audio(**response.arabic_audio.to_dict()))
        if response.translation_audio:
            ayah_response.translation_audio.MergeFrom(entity_proto.Audio(**response.translation_audio.to_dict()))
        if response.ayah_image:
            ayah_response.image.MergeFrom(entity_proto.Image(**response.ayah_image.to_dict()))
