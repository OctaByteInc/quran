...
def test_create_ayah():
    ...
    assert ProtoConverter.proto_to_dict(res.data) == ProtoConverter.proto_to_dict(ayah)


def test_find_ayah_by_id_without_parts():
    response = stub.FindAyahById(ayah_proto.AyahIdRequest(id='ayah-1'))

    assert response.data.ayah.id == 'ayah-1'
    assert response.data.ayah.number == 1
...
def test_find_ayah_by_id_with_parts():
    ...
    assert response.data.ayah.id == 'ayah-1'
    assert response.data.translation.id == 'translation-1'
    assert response.data.surah.id == 'surah-1'
    assert response.data.edition.id == 'edition-1'
    assert response.data.arabic_audio.id == 'audio-1'
    assert response.data.arabic_audio.type == 'Arabic'
    assert response.data.translation_audio.id == 'audio-2'
    assert response.data.translation_audio.type == 'Translation'
    assert response.data.image.image == 'link to image'


def test_find_ayah_by_surah_id_without_parts():
    response_stream = stub.FindAyahBySurahId(ayah_proto.AyahIdRequest(id='surah-1'))

    for response in response_stream.data:
...
def test_find_ayah_by_number_in_surah():
    ...
    assert response.data.ayah.number_in_surah == 1
